...
export function registerIpcHandlers(deps: IpcDeps) {
  ...
  ipcMain.handle("op:signin", async () => {
    return openTerminalCommand(
      'op signin && echo "\\n[Signed in. You can close this window and return to gus-mac.]"',
    );
  });
  ...
  ipcMain.handle("op:install-dependencies", async () => {
    return openTerminalCommand(
      [
        ...
        'echo "\\n[1Password and 1Password CLI install finished. Open 1Password, enable CLI integration if needed, then return to gus-mac.]"',
      ].join("; "),
    );
  });

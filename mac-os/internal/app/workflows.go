...
func (a app) factoryInstallPhases(opts options) []tui.Phase {
	return []tui.Phase{
		{Name: "factory install", Enabled: true, Run: func(w io.Writer) error { return a.runFactoryInstall(w, opts) }},
	}
}

func (a app) runFactoryInstall(w io.Writer, opts options) error {
	phases := []tui.Phase{
		{Name: "prerequisites", Run: func(w io.Writer) error { return a.withStdout(w).ensurePrerequisites(opts) }},
		{Name: "homebrew bundle", Run: func(w io.Writer) error { return a.withStdout(w).applyHomebrewBundle(opts) }},
		{Name: "app store apps", Run: func(w io.Writer) error { return a.withStdout(w).applyAppStoreApps(opts) }},
		{Name: "manual app report", Run: func(w io.Writer) error { return a.withStdout(w).reportManualApps(opts) }},
		{Name: "adopt safe dotfiles", Run: func(w io.Writer) error { return a.withStdout(w).adoptDotfiles(opts) }},
		{Name: "stow links", Run: func(w io.Writer) error { return a.withStdout(w).applyStow(opts) }},
		{Name: "macOS defaults", Run: func(w io.Writer) error { return a.withStdout(w).applyMacOSDefaults(opts) }},
		{Name: "doctor", Run: func(w io.Writer) error { return a.withStdout(w).runDoctor(opts) }},
	}

	for _, phase := range phases {
		fmt.Fprintf(w, "\n==> %s\n", phase.Name)

		if err := phase.Run(w); err != nil {
...
func (a app) tuiWorkflows() []tui.Workflow {
	...
	return []tui.Workflow{
		{Name: "Factory Install", Phases: a.factoryInstallPhases(factoryOpts)},
